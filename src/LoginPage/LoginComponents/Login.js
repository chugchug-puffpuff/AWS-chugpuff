...
const Login = ({ setAuthenticate, setUserName }) => {
  ...
  const loginUser = async (event) => {
    ...
    try {  // 로그인 엔드포인트
      const response = await axios.post('http://13.124.149.28:8080/api/login', { id, password });
      ...
      // 사용자 정보를 가져오는 엔드포인트
      const userResponse = await axios.get(`http://13.124.149.28:8080/api/members/username/${id}`, {
      ...
      // 알림 정보를 가져오는 엔드포인트
      const notificationsResponse = await axios.get('http://13.124.149.28:8080/api/calenders/notifications', {
        headers: { Authorization: `Bearer ${token}` }
      });
