...
const JobComment = ({ company, jobId, storedUserName, updateCommentCount }) => {
  ...
  const handleCommentSubmit = async () => {
    try {
      await axios.post(`http://13.124.149.28:8080/api/job-postings/${jobId}/comments`, {
        comment: comment,
      }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
  ...
  const handleConfirmDeleteClick = async () => {
    try {
      await axios.delete(`http://13.124.149.28:8080/api/job-postings/comments/${commentToDelete.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleConfirmEditClick = async () => {
    try {
      await axios.put(`http://13.124.149.28:8080/api/job-postings/comments/${commentToEdit.id}`, {
