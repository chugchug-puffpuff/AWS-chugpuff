...
const JobPostingList = ({ detailRegion, jobMidname, jobKeyword }) => {
  ...
  const fetchCommentsCount = useCallback(async (jobId) => {
    try {
      const response = await axios.get(`http://13.124.149.28:8080/api/job-postings/${jobId}/comments`, {
        headers: {
  ...
  useEffect(() => {
    ...
    const url = `http://13.124.149.28:8080/api/job-postings?jobMidname=${jobMidname}&jobName=${jobKeyword}&regionName=${detailRegion}&sort=${selectedType}`;
    fetchJobs(url);
  ...
  const handleSearch = async () => {
    ...
    const url = `http://13.124.149.28:8080/api/job-postings/search?jobMidname=${jobMidname}&jobName=${jobKeyword}&regionName=${detailRegion}&keywords=${searchKeyword}&sort=${selectedType}`;
    fetchJobs(url);
  };

  // 스크랩한 공고 목록 가져온 다음 리스트에 스크랩한 공고 표시
  useEffect(() => {
    axios.get('http://13.124.149.28:8080/api/job-postings/scraps', {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    })
