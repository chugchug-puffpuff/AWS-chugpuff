...
const RecruitInfo = ({ jobInfo, commentCount }) => {
  ...
  useEffect(() => {
    if (jobInfo && jobInfo.length > 0) {
      const fetchScrapCount = async () => {
        try {
          const response = await axios.get(`http://13.124.149.28:8080/api/job-postings/${jobInfo[0].jobId}/scrap-count`, {
            headers: {
  ...
  const handleScrapClick = async () => {
    try {
      await axios.post(`http://13.124.149.28:8080/api/job-postings/${jobInfo[0].jobId}/scrap`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const fetchScraped = async () => {
    try {
      const response = await axios.get('http://13.124.149.28:8080/api/job-postings/scraps', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
