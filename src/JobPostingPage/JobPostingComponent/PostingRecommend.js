...
const PostingRecommend = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://13.124.149.28:8080/api/job-postings/recommendations", {
          headers: {
        ...
        if (response.data.code === 4) {
          ...
          const jobRecommendations = await Promise.all(response.data.jobs.job.map(async job => {
            ...
            // 스크랩 수 불러오기
            const scrapResponse = await axios.get(`http://13.124.149.28:8080/api/job-postings/${job.id}/scrap-count`, {
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
              }
            });
