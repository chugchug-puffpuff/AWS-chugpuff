...
const BoardComment = ({ boardNo, storedUserName, updateCommentCount }) => {
  ...
  const fetchComments = useCallback(async () => {
    try {
      const response = await axios.get(`http://13.124.149.28:8080/api/comment/board/${boardNo}`, {
        headers: {
  ...
  const handleCommentSubmit = async () => {
    try {
      const response = await axios.post(`http://13.124.149.28:8080/api/comment?boardNo=${boardNo}`, {
        bcContent: comment,
      }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
  ...
  const handleConfirmDeleteClick = async () => {
    try {
      await axios.delete(`http://13.124.149.28:8080/api/comment/${commentToDelete.bcNo}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleConfirmEditClick = async () => {
    try {
      await axios.put(`http://13.124.149.28:8080/api/comment/${commentToEdit.bcNo}`, {
