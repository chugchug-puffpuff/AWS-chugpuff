...
const SeHistoryBar = ({ reload, isSavedClickedProp }) => {
  ...
  useEffect(() => {
    const fetchSelfIntroductionData = async () => {
      try {
        const response = await axios.get('http://13.124.149.28:8080/api/selfIntroduction/list', {
          headers: {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://13.124.149.28:8080/api/selfIntroduction/${selectedEsNo}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
