...
const CalendarComponent = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await fetch('http://13.124.149.28:8080/api/calenders', {
          headers: {
  ...
  const handleAddEvent = async () => {
    ...
    try {
      await axios.post('http://13.124.149.28:8080/api/calenders', {
  ...
  const handleEditEvent = async () => {
    ...
    try {
      if (selectedEvent && selectedEvent.memoNo) {
        await axios.put(`http://13.124.149.28:8080/api/calenders/${selectedEvent.memoNo}`, updatedEvent, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleDeleteEvent = async (eventToDelete) => {
    try {
      await axios.delete(`http://13.124.149.28:8080/api/calenders/${eventToDelete.memoNo}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
