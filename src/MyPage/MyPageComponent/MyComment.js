  ...
  useEffect(() => {
    const fetchJobComments = async () => {
      try {
        ...
        const commentResponse = await axios.get('http://13.124.149.28:8080/api/job-postings/comments/user', config);
        const jobIds = [...new Set(commentResponse.data.map(comment => comment.jobId))];

        const jobPromises = jobIds.map(jobId => 
          axios.get(`http://13.124.149.28:8080/api/job-postings/${jobId}`, config)
        );
