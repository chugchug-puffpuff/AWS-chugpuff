...
const EditInformation = ({setAuthenticate}) => {
  ...
  useEffect(() => {
    const fetchJobKeywords = async () => {
      try {
        const response = await axios.get(`http://13.124.149.28:8080/api/job-postings/job-names?jobMidName=${selectedJob}`);
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://13.124.149.28:8080/api/members/${user_id}/password?oldPassword=${currentPassword}`, {
        newPassword,
        confirmPassword
      }, {
        headers: {
  ...
  const selectJob = async (job) => {
    ...
    // 직무키워드 엔드포인트
    // try {
    //   const response = await axios.get(`http://13.124.149.28:8080/api/job-postings/job-names?jobMidName=${job}`);
    //   setJobKeywordList(response.data);
    // } catch (error) {
    //   console.error('직무 키워드 로드 에러:', error);
    // }
  ...
  const handleJobKeywordSubmit = async () => {
    ...
    try {
      await axios.put(`http://13.124.149.28:8080/api/members/${user_id}`, data, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleWithdraw = async () => {
    ...
    if (withdrawPassword === storedPassword) {
      try {
        await axios.delete(`http://13.124.149.28:8080/api/members/${user_id}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
