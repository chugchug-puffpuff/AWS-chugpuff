...
const SignUpPage = ({ authenticate, setAuthenticate }) => {
  ...
  const registerUser = async (userData) => {
    try {
      const response = await axios.post('http://13.124.149.28:8080/api/members', userData);
  ...
  const checkDuplicateId = async () => {
    ...
    if (!/^(?=.*[a-zA-Z])(?=.*[0-9]).{4,20}$/.test(formData.id)) {
      ...
      try {
        const response = await axios.get(`http://13.124.149.28:8080/api/members/checkUserId?id=${formData.id}`);
        if (response.data) {
