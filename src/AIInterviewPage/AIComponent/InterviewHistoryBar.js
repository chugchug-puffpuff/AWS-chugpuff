...
const InterviewHistoryBar = () => {
  ...
  const fetchInterviewData = async () => {
    ...
    try {
      const response = await axios.get(`http://13.124.149.28:8080/api/aiinterview/id/${id}`, {
        headers: {
      ...
      sortedDate.forEach(async (interview) => {
        const interviewResponse = await axios.get(`http://13.124.149.28:8080/api/aiinterview/${interview.aiinterviewNo}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://13.124.149.28:8080/api/aiinterview/${selectedInterviewId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
