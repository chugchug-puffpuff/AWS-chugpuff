...
const ImmInterview = ({ selectedType, selectedFeedback }) => {
  ...
  useEffect(() => {
    const fetchFirstQuestion = async () => {
      try {
        ...
        const audioInstance = new Audio(`/src/server/build/libs/output.mp3?timestamp=${new Date().getTime()}`);
  ...
  const fetchNextQuestion = async () => {
    ...
    try {
      ...
      const audio = new Audio(`/src/server/build/libs/output.mp3?timestamp=${new Date().getTime()}`);
      audio.play();
      setTimeout(() => {
        setIsQuestionTypingComplete(true);
        setIsQuestionTypingCompleteB(true);
  ...
  const convertAnswer = async () => {
    ...
    const filePath = `/src/server/build/libs/captured_audio_${aiinterviewNo}.wav`;
